...
while true; do
	[ `date +%s` -ge "$time_to_ping" ] && {
		for i in $interface ;do
			...
			if [[ "$device" =~ "Error Network device is not present" ]]; then
				...
				for h in $host ;do
					...
					[ "$debug" = "1" ]&& logger -s -p info -t icc "$i [ Device: $device ] [ Host: $h ] [ status: $STATUS ]"
		...
		json_dump >$ICC_STATE
		json_select "interface"
		[ "$debug" = "1" ]&& logger -s -p debug -t icc "Next ping: $(date --date @$(($interval + `date +%s`)) +%H:%M:%S)"
	}
	[ ! -z "$mode" ] && {
		for i in $interface ;do
			json_select $i
			...
			if [ "$status" == "online" ]; then
				...
				if [ "$panic" == "none" ]; then
					...
					json_get_var panic panic
					[ "$debug" = "1" ]&& logger -s -p debug -t icc "$i Panic $mode: [ $(date --date @$(($panic)) +%H:%M:%S) ]"
				else
